  ...
  doSearch() {
    if (this.search.length) {
      this.patientService.searchPatientByMobile(this.search).subscribe((paitentsArr: any) => {
        let patient = paitentsArr[0];
        let patientMobileNumber = patient.mobile_number;


        let filter = this.visits.filter(visit => visit.patient.mobile_number === patientMobileNumber)
        console.log(filter);
